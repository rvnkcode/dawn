...
impl Duration {
    pub fn new(entry: &Timestamp, now: i64) -> Result<Self, DurationError> {
        let from_secs = entry.as_seconds();
        // entry timestamp must not be in the future
        if now < from_secs {
            return Err(DurationError::InvalidDelta {
                from: from_secs,
                to: now,
            });
        }
        let from = DateTime::<Utc>::from_timestamp(from_secs, 0)
            .ok_or(DurationError::OutOfRange(from_secs))?;
        let to = DateTime::<Utc>::from_timestamp(now, 0).ok_or(DurationError::OutOfRange(now))?;
        Ok(Duration(format_age(from, to)))
    }
}
...
mod tests {
    ...
    fn seconds() {
        let now = 1_000_000;
        assert_eq!(
            Duration::new(&ts(now - 30), now).unwrap().to_string(),
            "30s"
        );
    }
    ...
    fn zero_delta() {
        let now = 1_000_000;
        assert_eq!(Duration::new(&ts(now), now).unwrap().to_string(), "0s");
    }
    ...
    fn minutes() {
        let now = 1_000_000;
        assert_eq!(
            Duration::new(&ts(now - 3 * MINUTE), now)
                .unwrap()
                .to_string(),
            "3min"
        );
    ...
    fn hours() {
        let now = 1_000_000;
        assert_eq!(
            Duration::new(&ts(now - 5 * HOUR), now).unwrap().to_string(),
            "5h"
        );
    }
    ...
    fn days() {
        let now = 100_000_000;
        assert_eq!(
            Duration::new(&ts(now - 10 * DAY), now).unwrap().to_string(),
            "10d"
        );
    }
    ...
    fn days_just_under_two_weeks() {
        let now = 100_000_000;
        assert_eq!(
            Duration::new(&ts(now - 13 * DAY), now).unwrap().to_string(),
            "13d"
        );
    }
    ...
    fn weeks_at_two_week_boundary() {
        let now = 100_000_000;
        assert_eq!(
            Duration::new(&ts(now - 14 * DAY), now).unwrap().to_string(),
            "2w"
        );
    }
    ...
    fn weeks_eleven() {
        let now = 100_000_000;
        assert_eq!(
            Duration::new(&ts(now - 11 * WEEK), now)
                .unwrap()
                .to_string(),
            "11w"
        );
    ...
    fn calendar_months_exact_three() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "3mo");
    }
    ...
    fn calendar_months_borrow_still_weeks() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "11w");
    }
    ...
    fn calendar_months_nine_across_year() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "9mo");
    }
    ...
    fn year_and_months() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "1y3mo");
    }
    ...
    fn year_exact_no_months() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "1y");
    }
    ...
    fn year_just_before_anniversary_shows_months() {
        ...
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "11mo");
    }
    ...
    fn two_years_no_months() {
        ...
        let to = ymd(2025, 6, 1);
        assert_eq!(Duration::new(&ts(from), to).unwrap().to_string(), "2y");
    }

    #[test]
    fn invalid_negative_delta() {
        let now = 1_000_000;
        let err = Duration::new(&ts(now + 100), now).unwrap_err();
        assert_eq!(
            err,
            DurationError::InvalidDelta {
                from: now + 100,
                to: now,
            }
        );

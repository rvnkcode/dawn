...
│   ├── task.rs              # Unit tests in #[cfg(test)] modules
│   └── task/
│       └── description.rs   # #[cfg(test)] mod tests { ... }
└── tests/                   # E2E tests
    ├── api_test.rs
    ├── db_test.rs
...
Unit tests go inside `#[cfg(test)]` modules in the same file. E2E tests go in `tests/`.
...
```bash
cargo llvm-cov --workspace --lib                              # Summary
cargo llvm-cov --workspace --lib --html                       # HTML report
cargo llvm-cov --workspace --lib --html --fail-under-lines 80 # Fail if below threshold
```
...
cargo test --workspace --test api_test       # Specific integration test (tests/api_test.rs)

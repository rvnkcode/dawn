...
The Task is the core entity in Taskwarrior.  
Each task has a unique UUID and various attributes.

### Implementation Notes (C++ Source)

- Attribute names are **string literals**, not `#define` constants
- Task internally stores attributes as `std::map<std::string, std::string>`
- Each attribute has a Column class in `src/columns/` for conversion/rendering
- See `src/Task.h` and `src/Task.cpp` for implementation details

...
pub fn parse_config(input: &str) -> Result<Config, ParseError> {
    toml::from_str(input).map_err(ParseError::from)
}
```
...
cargo llvm-cov --workspace --lcov > lcov.info # LCOV format for CI
cargo llvm-cov --workspace --fail-under-lines 80  # Fail if below threshold
```
...
cargo test --workspace --test api_test        # Integration tests only
cargo test --workspace --doc                  # Doc tests only
cargo test --workspace --no-fail-fast         # Don't stop on first failure
cargo test --workspace -- --ignored           # Run ignored tests
```

...
1. Run `cargo check --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --check`, and `cargo test --workspace` — if any fail, stop and report
...
- **External API without docs**: items re-exported from the crate root (`lib.rs`) missing `///` documentation. `pub(crate)` and internal `pub` items are exempt — per CLAUDE.md, avoid comments whose content is already obvious from identifiers
...
cargo build --workspace --release 2>&1 | head -50
```
